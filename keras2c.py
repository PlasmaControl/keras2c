...
def write_weights_LSTM(layer,file, model_io):
    ...
    s += 'int ' + layer.name + '_return_sequences = ' + str(return_sequences) + ';\n'
...
def write_weights_Dense(layer,file, model_io):
    ...
    s = 'float ' + layer.name + '_fwork[' + str(insize+np.prod(A.shape)) + '] = {0}; \n'
    file.write(s)
    file.write('\n \n')
...
def write_weights_Conv1D(layer,file, model_io):
    ...
    for i,(inp,outp) in enumerate(zip(inputs,outputs)):
        ...
        file.write(array2c(np.zeros(inrows+pad_top+pad_bottom,incols),layer.name + '_padded' + str(i) + '_input'))
...
def write_weights_Reshape(layer,file,model_io):
    ...
    s += 'size_t' + nm + '_newshp[K2C_MAX_NDIM] = {' + str(np.array2string(shp.astype(int),separator=',')[1:-1]) + '}; \n'
    file.write(s)
...
def write_weights_Permute(layer,file,model_io):
    ...
    for i,(inp,outp) in enumerate(zip(inputs,outputs)):
        ...
        s = 'size_t ' + nm + '_permute[' + str(permute.size) + '] = {' + str(np.array2string(permute.astype(int),separator=',')[1:-1]) + '}; \n'
        file.write(s)
...
def write_weights_Dot(layer,file,model_io):
    ...
    inputs, outputs = get_layer_io_names(layer)
    for i,(inp,outp) in enumerate(zip(inputs,outputs)):
        work_size = np.prod(layer.get_input_at(i)[0].shape[1:]) + np.prod(layer.get_input_at(i)[1].shape[1:])
        if outp not in model_io[1]:
            file.write(array2c(np.zeros(outshp),outp + '_output'))   
...
def write_layer_AdvancedActivation(layer,file,inputs,outputs,i):
    ...
    if layer_type(layer) == 'LeakyReLU':
        s = 'keras2c_LeakyReLU(' + inputs + '.array,' + inputs + '.numel,' + nm + '_alpha); \n'
    if layer_type(layer) == 'PReLU':
        s += 'keras2c_PReLU(' + inputs + '.array,' + inputs + '.numel,' + nm + '_alpha); \n'
    if layer_type(layer) == 'ELU':
        s = 'keras2c_ELU(' + inputs + '.array,' + inputs + '.numel,' + nm + '_alpha); \n'
    if layer_type(layer) == 'ThresholdedReLU':
        s = 'keras2c_ThresholdedReLU(' + inputs + '.array,' + inputs + '.numel,' + nm + '_theta); \n'
    if layer_type(layer) == 'ReLU':
...
def write_dummy_layer(layer,file,inputs,outputs,i):
    s = 'k2c_tensor *' + outputs + ' = &' + inputs + '; // layer only does something during training, during predict just rename for clarity \n'
    file.write(s)
